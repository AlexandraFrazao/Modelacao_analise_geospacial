...
CFR_data = pd.read_pickle('CFR_data.pickle')

#Drop unecessary variables

CFR_data = CFR_data.drop(columns=['Country of Registration',
                                   'UVI',
...
# Add a new column 'Active' based on the year in 'Event End Date'
CFR_data['Active'] = [1 if boat.year==2100 else 0 for boat in CFR_data['Event End Date']]
...
CFR_data.to_pickle('CFR_data_cut.pickle')
...
Names_unique.to_pickle('grouped_CFR.pickle')
Names_unique.to_excel('grouped_CFR.xlsx')
...
loc_to_loc_pair.to_pickle('loc_to_loc_pair.pickle')
loc_to_loc_pair.to_excel('loc_to_loc_pair.xlsx')

# Save the CFR_to_local DataFrame to a pickle file
CFR_to_local.to_pickle('CFR_to_local.pickle')
CFR_to_local.to_excel('CFR_to_local.xlsx')